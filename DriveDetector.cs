...
namespace Dolinay
{
    ...
    ///
    /// NOTE: this class no longer requires being created on a gui thread
    /// (running a message pump). it just creates a native window to listen for
    /// windows messages and spins its own pump on a bg thread.
    ///
    /// </summary>
    public class DriveDetector : IDisposable
    {
        ...
        /// </summary>
        public DriveDetector() : this(null)
        {
        }
        ...
        public DriveDetector(string FileToOpen)
        {
            ...
            if (WinUtils.WindowHelper.ApplicationWillCreateWindow)
            {
                WinUtils.WindowHelper.CreatedWindow += _GotWindow;
                if (WinUtils.WindowHelper.Handle != IntPtr.Zero)
                    _GotWindow(WinUtils.WindowHelper.Handle);
            }
            else
            {
                WinUtils.WindowHelper.CreatedWindow += _GotWindow;
                WinUtils.WindowHelper.CreateHiddenWindow();
                if (WinUtils.WindowHelper.Handle != IntPtr.Zero)
                    _GotWindow(WinUtils.WindowHelper.Handle);
            }
        }

        void _GotWindow(IntPtr hwnd)
        {
            WinUtils.WindowHelper.CreatedWindow -= _GotWindow;
            Init(hwnd);
            WinUtils.WindowHelper.WndProc += WndProc;
        }
        ...
        private const int DBT_CUSTOMEVENT = 0x8006;   // custom event -- may be preparing to remove
        private const int DBT_DEVTYP_VOLUME = 0x00000002; // drive type is logical volume
        private static Guid GUID_IO_VOLUME_LOCK = // custom event with this guid is fired before Eject is attempted
            new Guid(0x50708874, 0xC9AF, 0x11D1, 0x8F, 0xEF, 0x00, 0xA0, 0xC9, 0xA0, 0x6D, 0x32);
        ...
        private class Native
        {
            ...
            [DllImport("ole32.dll")]
            public static extern bool IsEqualGUID([In] ref Guid rguid1, [In] ref Guid rguid2);
